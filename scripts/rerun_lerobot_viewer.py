...
SMPL_JOINTS_SLICE = slice(0, 72)
SMPL_POSE_SLICE = slice(72, 135)
LEFT_TRIGGER_IDX = 135
RIGHT_TRIGGER_IDX = 136
BODY_ROT6D_SLICE = slice(137, 143)
...
def parse_action_layout(
    ...
    if actions.shape[1] <= RIGHT_TRIGGER_IDX:
        raise ValueError(
            f"SMPL layout requires action_dim >= {RIGHT_TRIGGER_IDX + 1}, got {actions.shape[1]}"
        )
    smpl_joints = actions[:, SMPL_JOINTS_SLICE].reshape(actions.shape[0], 24, 3)
    smpl_pose = actions[:, SMPL_POSE_SLICE]
    left_trigger = actions[:, LEFT_TRIGGER_IDX]
    right_trigger = actions[:, RIGHT_TRIGGER_IDX]
    body_rotmat = None
    if actions.shape[1] >= BODY_ROT6D_SLICE.stop:
        body_rot6d = actions[:, BODY_ROT6D_SLICE]
...
def main() -> None:
    ...
    if args.smpl_viz:
        smpl_joints, smpl_pose, left_trigger, right_trigger, body_rotmat = parse_action_layout(actions)
    ...
    left_video = get_video_path(dataset_dir, info, episode_index, "left")
    right_video = get_video_path(dataset_dir, info, episode_index, "right")
    ego_video = get_video_path(dataset_dir, info, episode_index, "egocentric")
    ...
    if left_video.exists() and right_video.exists():
        left_frames = read_all_frames(left_video)
        right_frames = read_all_frames(right_video)
        mode = "stereo"
    elif ego_video.exists():
        ego_frames = read_all_frames(ego_video)
        mode = "mono"
    else:
        raise FileNotFoundError(
            f"No video found for episode {episode_index}. Tried:\n{left_video}\n{right_video}\n{ego_video}"
        )

    lengths = [len(actions)]
    if mode == "stereo":
        lengths += [len(left_frames), len(right_frames)]
    else:
        lengths += [len(ego_frames)]
    n = int(min(lengths))
    if n <= 0:
        ...
        rr.TextDocument(
            "\n".join(
                [
                    ...
                    f"action_layout=[0:72]=smpl_joints, [72:135]=smpl_pose, [135]=left_trigger, [136]=right_trigger, [137:143]=body_rot6d(optional)",
    ...
    for i in range(n):
        ...
        if mode == "stereo":
            rr.log("camera/left", rr.Image(left_frames[i]))
            rr.log("camera/right", rr.Image(right_frames[i]))
        else:
            rr.log("camera/egocentric", rr.Image(ego_frames[i]))

        for d in dims:
            log_scalar_compat(f"action/dim_{d:03d}", float(actions[i, d]))
        if args.smpl_viz and smpl_joints is not None and smpl_pose is not None:
    ...
    if args.smpl_viz:
        ...
        print("SMPL paths: smpl/joints, smpl/bones, smpl/end_effectors, smpl/pose/*, smpl/trigger/*")
    if args.save:
