...
def infer_layout(actions: np.ndarray, layout_arg: str) -> str:
    if layout_arg in {LAYOUT_72, LAYOUT_86, LAYOUT_143}:
        return layout_arg
    if layout_arg != LAYOUT_AUTO:
        raise ValueError(f"Unknown layout '{layout_arg}', expected one of: auto|72|86|143")
    ...
    if d >= 72:
        return LAYOUT_72
    raise ValueError(f"auto layout cannot infer from action_dim={d}; expected >=72")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default="auto",
        help="action layout: auto|72|86|143",
    )
